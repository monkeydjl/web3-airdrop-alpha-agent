...
`docs/OPERATIONS.md`、`docs/OBSERVABILITY.md`、`docs/DATA_SOURCE_STRATEGY.md`，
合计 1116 处。每个 3 字节中文字符的**第 3 字节被替换成 '?'**，前 2 字节完好。
文件因此变成非法 UTF-8 —— 至少还能被机械检测出来。
且前 2 字节把候选字符限制在 64 个以内，修复是"做选择题"。

### 二型：整字被替换成 '?'（**仍是合法 UTF-8**）

`docs/API_SPEC.md` 70 处（原始命中 129，其中 59 处是代码块误报）。
...
def blank_code_blocks(text: str) -> str:
    ...
    **必须排除代码**：mermaid 流程图里 `{全绿?}`、`{需立即修复?}` 是正常写法，
    ...
    以及本文件自己的文档字符串 4 处）全部归零，只剩真正损坏的 API_SPEC.md。
