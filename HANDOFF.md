...
## 编码损坏：三型全部修完（2026-08-23）

**1188 处全部处理完毕，三份豁免清单同时清空。** 一型 1116（OBSERVABILITY 214 +
OPERATIONS 404 + DATA_SOURCE_STRATEGY 498）、二型 70、三型 2。

### 修法：不是逐字节恢复，是按实测重写

三份一型文档都是**对着运行中的代码重新写一遍**，再用 pytest 双向钉住：
`test_observability_doc_parity.py`（17 条）、
`test_operations_doc_parity.py`（39 条）、
`test_data_source_strategy_parity.py`（23 条）。

### 那套自动修复工具，最终一处也没用于交付

`scripts/repair_utf8_docs.py` 最多能定 56.4%（629/1116），剩下 487 处是
"人工选择题"。但做那道题的收益是负的：填对了读者也无法验证，
填错了则永久混入原文。**工具的价值是量出「自动修复不可行」这个结论** ——
每处第 3 字节有 64 个合法取值，同文档字符集收敛只能唯一确定 9.1%
（101/1115），结构标尺在 2–5 字窗口下确定 0 处，且损坏还吞掉了相邻字节。

工具留在仓库里作为这条证据链，**但它不是推荐路径**。

### 内容失真远多于编码损坏（这一轮最一致的发现）

五份挂在豁免清单上的文件，内容错误全都比乱码严重：
35 个不存在的指标（OBSERVABILITY）、16 个幽灵端点（API_SPEC）、
18 个幽灵指标 + 2 个虚构巡检脚本 + 一条不存在的「LLM 超预算自动停用」
（OPERATIONS）、47 个与代码不符的配置键（`.env.example`）、
10 个全错的采集器路径 + 一条代码里不存在的 `discovery_score` 统一公式
（DATA_SOURCE_STRATEGY）。

它们能活这么久只因为**「反正这文件已登记待修」，于是没人逐行读**。
**登记豁免掩盖的是内容问题，不只是字节问题。**
...
（实测 500 个文本文件、0 处损坏）。往清单里加文件会当场变红。
pre-commit 钩子三型都拦。详见 `docs/ENCODING_REPAIR.md`。

### 三型的处理原则值得单独记

`docs/SYSTEM_DIRECTION_CHANGE.md` 丢的是两个小节标题的装饰 emoji，
没有底本、选哪个只能靠猜。**最终决定：直接删掉，不补猜的。**
补一个「看起来差不多」的 emoji 会让这份文档看起来**从未损坏过**，
下一个人再也分不清哪个是原作者选的。
**一个看不出来的猜测比一个看得出来的缺口更糟。**

### 二型为什么绝不能自动修

一个 `?` 不总是只吃掉一个字符 —— 逐行对齐底本后，24 处可核对位置里
恰好 1 个字符的是 **0 处**、多于 1 个的是 **24 处**（例如一个 `?` 吃掉了
`求/` 两个字符）。既没有候选集约束（候选是全部汉字），也没有长度约束，
所以**无法用逐字节校验证明修复没越界**。
...
- [x] ~~**继续编码修复**~~ → **2026-08-23 全部完成**：三型共 1188 处修完，
      三份豁免清单同时清空，各有一条测试正面钉住必须为空 + 三条全仓扫描
      （500 个文本文件、0 处损坏）。三份一型文档都是**按实测重写**而非
      逐字节恢复，各配一套 parity 门禁（17 / 39 / 23 条）
...
2. ~~**编码损坏只剩一批**~~ → **三型全部修完（1188 处），三份豁免清单清空。**
   三份一型文档按实测重写并各配 parity 门禁；一型现在也是零豁免硬门禁。
3. **括号与句号两条推断规则仍不是 100%**（99.57% / 99.37%）——
   但它们**最终一处也没用于交付**，三份文档全部按实测重写。
   规则留在工具里仅作为「自动修复不可行」的证据。
