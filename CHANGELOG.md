...
**发现的问题**：3 个文档存在 UTF-8 编码损坏，合计 **1116 处**中文字符丢字
（`docs/DATA_SOURCE_STRATEGY.md` 498 处、`docs/OPERATIONS.md` 404 处、
`docs/OBSERVABILITY.md` 214 处）。损坏形态是每个 3 字节中文字符的第 3 字节被
替换成 `?` —— 典型的"以非 UTF-8 编码写回文件"造成的不可逆丢失。

这类损坏**静默**：文件照样能打开、git 照常提交，只是内容里多了一堆 `?`。
它在 git 历史里潜伏了 3 个提交才被发现，且 `DATA_SOURCE_STRATEGY.md` 的
**所有历史版本都已损坏**，无法从 git 恢复。

**已修复 482 处（43%）**，全部是可证明的恢复，不含猜测：
- 432 处由 `6823d18` 的干净历史底本序列对齐**精确恢复**
- 26 处由"该前缀在全仓语料里只对应唯一字符"推断
- 24 处由结构约束推断（架构图框线：横线只能接横线）

**刻意未修的 634 处**：这些位置的前缀对应多个候选字符，无法证明是哪一个。
实测 `efbc` 前缀里 `（）：，` 各占 26/25/20/19% —— 按频率猜的错误率接近 3/4。
把猜测写进文档比留占位符更糟：读者无法分辨哪句是原文、哪句是机器编的。
已导出为受约束选择题清单（每条给候选集 + 前后 40 字上下文），填写者只能在
候选里挑，且答案要过逐字节校验。

**新增三个工具**：
- `scripts/check_encoding.py` — 拦截非法 UTF-8，**已挂 pre-commit 钩子**防复发
- `scripts/repair_utf8_docs.py` — 四轮修复 + 选择题导出/合并
