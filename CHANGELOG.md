...
**一型已修复 646 处（58%）**，全部是可证明的恢复，不含猜测：
- 432 处由 `6823d18` 的干净历史底本序列对齐**精确恢复**
- 27 处由"该前缀在全仓语料里只对应唯一字符"推断
- 187 处由上下文规则推断，并通过**交叉验证**：规则与底本对齐独立推断同一批
  位置，105 处可核对、**冲突 0**

**刻意未修的一型 470 处**：这些位置的前缀对应多个候选字符，无法证明是哪一个。
...
- `scripts/verify_utf8_repair.py` — 机械校验：未损坏正文必须逐字节一致，
  ...
  只有第一种通过，其余全部拦下
- `backend/tests/test_utf8_repair_tools.py`（24 个）— 含上述交叉验证
...
括号 99.6%、句号 99.4% —— **小样本给了虚假的安全感**。本次输出因交叉验证
保护而未受影响，但规则本身待收紧，已如实记入 `docs/ENCODING_REPAIR.md` §4 与
§6「还没做的事」。
