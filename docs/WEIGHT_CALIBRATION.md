...
### 4.1.1 资格门不参与拟合（ADR-015）

资格门（`app/agents/eligibility.py`）产出的 `veto` **只改 label，不改 score**，
并且**不是可拟合参数**：搜索空间只含 §4.2 的八个权重与 FARM/WATCH 阈值，
不允许把否决规则的开关或阈值放进 J 的优化变量。

原因是两者回答的不是同一个问题。score 表达「这个项目本身好不好」，
veto 表达「现在还有没有能参与的路径」。把否决拿去拟合，等于允许优化器
为了几个点的 J 把「已发币、没有后续分配」重新算成机会 —— 那不是校准，
是把业务约束折价卖掉。

由此产生两条硬约束：

1. 重加权（§4.3 step 2）只重算加权总分与由分数推出的 label，**veto 保持原值**。
2. 回测报告必须单独看 `veto_false_negatives`（被否决但实际曾有空投的样本数）。
   它不能混进 FPR：普通误报是白投入，否决误判是**直接把真机会扔掉**，
   代价完全不同。这个数字大于 0 就要逐条查规则，而不是调权重。

### 4.1.2 校准前必须确认没有维度退化成常数

**调权重之前先确认每一维真的在区分项目。** 一个取值恒定的维度，无论权重定多少
都对排序毫无贡献 —— 优化器会把这份权重的"额度"转移到别的维度上，拟合出的结果
看似 J 提升了，实际是在用错误的输入拟合。

已知踩过一次：`narrative_timing` 的输入完全来自 `SECTOR_PROFILE[sector]` 查表，
而查表原先是大小写敏感精确匹配 + 未命中静默走默认档，默认档恰好让该维**恒等于
60.0**。M2 回测的 19 个样本全部踩中，等于用七维模型冒充八维。

现在 `resolve_sector_profile()` 做了写法归一，未命中会打
`narrative.sector_profile_missing`（见 `OBSERVABILITY.md §2.2`）。
**校准启动前的检查动作**：拉一遍待校准样本的子分，确认没有哪一维的方差接近 0；
并检查该 warning 的出现频次，持续出现说明有一批项目的 `narrative_timing` 仍是
常数，这批样本的该维信息量为零。
