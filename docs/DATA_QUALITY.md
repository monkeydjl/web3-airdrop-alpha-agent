...
| `dune` | 0.8 | 链上可信（**未接入**，见下注） | 中高 |
| `cryptorank` | 0.75 | 商业库，偶有延迟 | 中 |
| `twitter` | 0.5 | 噪音大，仅作信号 | 低 |

> `dune` 一行是**将来接入时的预设权重**，当前无 collector、不产生任何数据。
> 配置键 `DUNE_API_KEY` 已于 2026-09-03 删除（详见 DATA_SOURCE_STRATEGY §8.2）。
> 权重留着是因为它反映的是"链上数据比商业库可信"这个判断，与是否已接入无关。

**v2.0 新增源（ADR-012）**：

| 源 | reliability | 理由 | 冲突仲裁优先级 |
| --- | --- | --- | --- |
| `github` | 0.85 | 代码活跃度客观可验证 | 高 |
| `coingecko` | 0.9 | 代币状态权威源 | 高 |
| `chain` | 0.8 | 链上数据客观 | 中高 |
| `galxe`/`layer3` | 0.7 | 任务平台，可能有刷量 | 中 |
| `manual` | 1.0 | 人工输入，但覆盖面有限 | 最高（仅对手动输入项目） |
...
### 6.1 采集质量维度（v2.0，ADR-012）

> 自动扫描模式下，除本规范的 6 维数据质量外，还需关注采集管道自身的质量。
> 详见 `DATA_SOURCE_STRATEGY.md §9.1`。
>
> ⚠️ **下表是目标设计，不是已实现的度量。** 实际实现的只有 5 个告警阈值
> （`CollectionMetrics.check_alerts()`，命中写 `collection.alert` 警告日志）：
> `success_rate 0.95` / `avg_latency_ms 30000` / `freshness_minutes 120` /
> `coverage_rate 0.5` / `duplicate_rate 0.5`。
> **误报率、漏报率、去重准确率三项没有任何实现**，也没有"每周/每月"的统计任务。

| 采集质量维度 | 目标值 | 度量频率 | 是否已实现 | 对应数据质量维度 |
| --- | --- | --- | --- | --- |
| 误报率（不该进分析却进了） | <10% | 每周 | ❌ 无实现 | 准确性 |
| 漏报率（该发现却没发现） | <15% | 每月 | ❌ 无实现 | 完整性 |
| 信号新鲜度（信号产生到入库延迟） | <1 小时 | 每日 | ✅ 阈值 120 分钟 | 时效性 |
| 源覆盖率（≥2 源命中的项目占比） | ≥30% | 每周 | ✅ 阈值 0.5 | 一致性（多源交叉验证） |
| 采集稳定性（采集成功率） | ≥95% | 每日 | ✅ 阈值 0.95 | —（运维指标） |
| 去重准确率（1 - 误合并 - 漏合并） | ≥95% | 每周 | ❌ 无实现（有 `duplicate_rate 0.5` 阈值，量的是重复占比，不是准确率） | 唯一性 |

- 采集质量报告每月输出至 `evaluation/collection/YYYY-MM.md`
  —— ⚠️ **该目录不存在，此项从未执行过**（见 `OPERATIONS.md §11`
  与 `DATA_SOURCE_STRATEGY.md §11`）
