...
> 自动扫描模式下，除本规范的 6 维数据质量外，还需关注采集管道自身的质量。详见 `DATA_SOURCE_STRATEGY.md §采集质量评估指标`。

| 采集质量维度 | 目标值 | 度量频率 | 对应数据质量维度 |
| --- | --- | --- | --- |
| 误报率（不该进分析却进了） | <10% | 每周 | 准确性 |
| 漏报率（该发现却没发现） | <15% | 每月 | 完整性 |
| 信号新鲜度（信号产生到入库延迟） | <1 小时 | 每日 | 时效性 |
| 源覆盖率（≥2 源命中的项目占比） | ≥30% | 每周 | 一致性（多源交叉验证） |
| 采集稳定性（采集成功率） | ≥95% | 每日 | —（运维指标） |
